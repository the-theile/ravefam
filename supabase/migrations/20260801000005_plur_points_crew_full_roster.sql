-- PLUR Points Phase 3: crew_full_roster (Unity) -- "every open slot in a
-- crew gets claimed." Added to claim_and_merge_raver()'s direct-claim
-- branch (same reasoning as Phase 2: this is the one moment a stub's
-- claimed_by actually transitions, so it's the only reliable place to
-- notice a roster just became fully claimed). Same function signature as
-- the Phase 2 version -- CREATE OR REPLACE keeps grants intact, no new
-- overload risk this time.
...
begin
  ...
  -- PLUR Points: if this claim just completed a crew's roster (every
  -- member now claimed), credit the crew's leader.
  for v_crew_id in
