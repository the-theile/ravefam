-- ===== PM DASHBOARD v5: crew activation funnel + recent micro-feedback =====
-- Surfaces the instrumentation added in 20260809000000_crew_activation_analytics.sql
-- and 20260811000000_micro_feedback_responses.sql inside the existing Product
-- Metrics dashboard, instead of requiring a separate `select
-- get_crew_activation_funnel();` query. Adds two top-level keys to the
-- existing get_pm_dashboard_metrics() JSONB result, built on top of the
-- 20260806000001 version:
--   - crew_activation: activation_rate (for a new KPI card) + the same
--     8-week weekly cohort funnel get_crew_activation_funnel() returns
--   - recent_feedback: the latest 12 micro-feedback responses, for a
--     qualitative "what are people telling us" section
-- Everything else in the function body is unchanged from 20260806000001.
...
begin
  ...
  select jsonb_build_object(
    ...
    'crew_activation', jsonb_build_object(
      'activation_rate', (
        select case when sum(crews_created) > 0 then round(100.0 * sum(activated) / sum(crews_created), 0) else null end
        from activation_funnel
      ),
      'crews_created_8w', (select coalesce(sum(crews_created), 0) from activation_funnel),
      'activated_8w', (select coalesce(sum(activated), 0) from activation_funnel),
      'weekly', (
        select coalesce(jsonb_agg(jsonb_build_object(
        ...
        from weeks w left join activation_funnel f on f.cohort_week = w.week_start
      )
    ),
    'recent_feedback', (
      select coalesce(jsonb_agg(jsonb_build_object(
        'milestone', milestone, 'question', question, 'response', response,
        'crew_id', crew_id, 'created_at', created_at
